...
/** Derives the step id from a pathname like /finance/forecast. */
...
/**
 * The Finance suite: six agents told as one financial month.
 *
 * Same engine as Compliance. The one structural difference is the palette:
 * the root carries data-suite="finance", which re-points every colour token
 * to the "Tape" fork in globals.css. No component below knows which suite it
 * is in — they compose through the same rgb(var(--x-ch) / a) tokens.
 *
 * No ?step= back-compat: /finance is a new route with no legacy links.
 */
    ...
    <div data-suite="finance" className="min-h-screen bg-background">
      ...
      >
        ...
        <div key={`${stepId}-${runId}`} className="demo-step-in">
          ...
          {stepId === 'reconcile' && <StepReconcile />}
          {stepId === 'examine' && <StepExamine />}
          {stepId === 'anomaly' && <StepAnomaly />}
          {stepId === 'forecast' && <StepForecast />}
          {stepId === 'reporter' && <StepReporter />}
        </div>
