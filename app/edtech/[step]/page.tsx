...
import { agent, readyStep, steps } from '@/demos/edtech/config';
...
export function generateMetadata({
  ...
  return {
    title: step
      ...
      : `${agent.name} — six agents, one term · Cynea Demo`,
    description: step ? step.title : agent.tagline,
  };
}

export default function EdtechStepPage({ params }: { params: { step: string } }) {
  if (!isValidStep(params.step)) notFound();
  return <EdtechDemo initialStep={params.step} />;
}
